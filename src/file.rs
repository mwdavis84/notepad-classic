...
fn encode_body_to_writer(
    ...
    while index < edit_text.len() {
        ...
        if unit == b'\r' as u16 || unit == b'\n' as u16 {
            ...
            for separator_unit in newline_units(format.newline) {
                write_char(
                    writer,
                    char::from_u32(u32::from(*separator_unit)).unwrap(),
                    format.encoding,
                )?;
            }
        } else {
            ...
            write_char(writer, decoded, format.encoding)?;
        }
        index += 1;
    }
    Ok(())
...
fn write_char(writer: &mut impl Write, ch: char, encoding: Encoding) -> io::Result<()> {
    match encoding {
